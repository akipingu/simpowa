...
knitr::opts_chunk$set(
  collapse = TRUE,       # collapse output and code together
  comment = "#>",        # prefix output lines with "#>"
  fig.path = "man/figures/README-",  # where to save figures
  out.width = "100%",     # scale figures to full width
...
A simpowa R package simulates mosquito count data under short- and long-term semi-field experimental design scenarios, evaluating single or combined interventions. The package incorporates fixed treatment effects, random chamber-level or sampling time-level variability to support robust power analysis for short- or long-term semi-field experimental planning. 
...
If you use **simpowa** in your research or publications, please cite it appropriately as follows:
```{r}
#cite this package
citation("simpowa")
```
...
```{r}
# Estimate power, account for random effects using "use.random = TRUE".
# Estimate power, account for fixed effect only using "use.random = FALSE".
